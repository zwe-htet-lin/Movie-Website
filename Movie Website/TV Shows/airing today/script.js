...
// Show movies on body
const url = "https://api.themoviedb.org/3/tv/airing_today?api_key=845e81600d9d504ee9e6e2f1b08dee9f&page=1";
...
const showMovies = async (url) => {
    ...
    const tvShows = responseData.results;
    ...
    for (let i = 0; i < tvShows.length; i++) {
        ...
        movieBox.setAttribute("tv-id",`${tvShows[i].id}`);

        let tvPoster = tvShows[i].poster_path;
        if(tvPoster != null){
            tvPoster = imgpath + tvPoster;
        }else{
            tvPoster = "../../images/image_not_found.png"
        }

        let day, month, year, date;
        if(tvShows[i].first_air_date === undefined){
            day = "";
            month = "";
            year = "";
            ...
            day = (tvShows[i].first_air_date).substring(8,10);
            month = (tvShows[i].first_air_date).substring(5,7);
            year = (tvShows[i].first_air_date).substring(0,4);
        ...
        let ratingDisplay;
        if(tvShows[i].vote_average === undefined || tvShows[i].vote_average === 0){
            ratingDisplay = "none";
        }else{
            ratingDisplay = "flex";
        };
        ...
        movieBox.innerHTML = `
            ...
            <div class="movieText">
                <h3>${tvShows[i].name}</h3>
                <div class="movieInfo">
                    <p>${date}</p>
                    <div style="display: ${ratingDisplay};" class="rating">
                        <span class="${ratingColor(tvShows[i].vote_average)}">${tvShows[i].vote_average}</span>
        ...
        movieImg.addEventListener("click", () => {
            showMovieDetails("tv",tvShows[i].id);
            const object = { mediaType: "tv", tmdbId: `${tvShows[i].id}`, searchText: "" };
            array.push(object);
        });
...
const showSearchListItem = async (searchText) => {
    ...
    const url = `https://api.themoviedb.org/3/search/tv?&api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}`;
    const response = await fetch(url);
    const responseData = await response.json();
    ...
    for(let i = 0; i < tvShows.length; i++){
        ...
        searchListItem.setAttribute("tv-id", `${tvShows[i].id}`);
        ...
        if(tvShows[i].first_air_date === undefined){
            ...
            firstAirDate = tvShows[i].first_air_date.substring(0,4);
        };
...
back.addEventListener("click", () => {
    ...
    if(array[array.length-2].mediaType === "" && array[array.length-2].tmdbId === "" && array[array.length-2].searchText === ""){
        ...
        headingH1.innerHTML = "TV Shows Airing Today";
        ...
        const url = `https://api.themoviedb.org/3/search/tv?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${array[array.length-2].searchText}`;
        showMovies(url);
...
const showMoreSearchResult = async (searchText) => {
    ...
    if(searchText){    
        const searchResultUrl = `https://api.themoviedb.org/3/search/tv?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}&page=1`;
...
next.addEventListener("click", () => {
    ...
    if(movieContainer.getAttribute("search-text") === null){
        const url = `https://api.themoviedb.org/3/tv/airing_today?api_key=845e81600d9d504ee9e6e2f1b08dee9f&page=${index}`;
        showMovies(url);
    }else{
        const searchText = movieContainer.getAttribute("search-text");
        const url = `https://api.themoviedb.org/3/search/tv?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}&page=${index}`;
        showMovies(url);
    }
});
