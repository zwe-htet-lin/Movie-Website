...
// Show people on body
const url = `https://api.themoviedb.org/3/person/popular?api_key=845e81600d9d504ee9e6e2f1b08dee9f&page=1`;
const imgpath = "https://image.tmdb.org/t/p/w1280";

const showPeople = async (url) => {
    const response = await fetch(url);
    const responseData = await response.json();

    const people = responseData.results;
    ...
    for (let i = 0; i < people.length; i++) {
        ...
        personBox.setAttribute("person-id",`${people[i].id}`);

        let profilePoster = people[i].profile_path;
        if(profilePoster != null){
            profilePoster = imgpath + profilePoster;
        }else{
            profilePoster = "../images/image_not_found.png"
        };

        const knownFor = people[i].known_for;
        const p = document.createElement("p");
        let span;
        for(let j = 0; j < knownFor.length; j++){
            span = document.createElement("span");

            let name;
            if(knownFor[j].title === undefined){
                name = knownFor[j].name + ", ";
            };
            if(knownFor[j].name === undefined){
                name = knownFor[j].title + ", ";
            };
        ...
        img.src = profilePoster;

        const personText = document.createElement("div");
        personText.classList.add("personText");

        const h3 = document.createElement("h3");
        h3.innerHTML = people[i].name;

        personImg.append(img);
        personText.append(h3, p);
        personBox.append(personImg, personText);
        peopleContainer.append(personBox);

        personImg.addEventListener("click", () => {
            showPersonDetails(people[i].id);
            const object = { mediaType: "person", tmdbId: `${people[i].id}`, searchText: "" };
            array.push(object);
        });
...
const showSearchListItem = async (searchText) => {
    ...
    const url = `https://api.themoviedb.org/3/search/person?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}`;
    const response = await fetch(url);
    const responseData = await response.json();
    ...
    for(let i = 0; i < people.length; i++){
        ...
        searchListItem.setAttribute("person-id", `${people[i].id}`);
        ...
        let role = people[i].known_for_department;
        if(role === "Acting"){
            role = "Actor";
        ...
        let knownFor;
        if(people[i].known_for.length === 0){
            knownFor = "";
        }
        else if(people[i].known_for[0].title === undefined){
            knownFor = people[i].known_for[0].name;
        }
        else if(people[i].known_for[0].name === undefined){
            knownFor = people[i].known_for[0].title;
        };
...
// Show search people on body when submit
searchBox.addEventListener("submit",(event) => {
    ...
    peopleContainer.setAttribute("search-text", `${searchText}`);
...
back.addEventListener("click", () => {
    ...
    if(array[array.length-2].mediaType === "" && array[array.length-2].tmdbId === "" && array[array.length-2].searchText === ""){
        ...
        headingH1.innerHTML = "Popular People";

        showPeople(url);
        array.pop();

        index = 1;
        peopleContainer.removeAttribute("search-text");
        ...
        peopleContainer.setAttribute("search-text", `${array[array.length-2].searchText}`);

        const url = `https://api.themoviedb.org/3/search/person?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${array[array.length-2].searchText}`;
        showPeople(url);
...
const showMoreSearchResult = async (searchText) => {
    if(searchText){    
        const searchResultUrl = `https://api.themoviedb.org/3/search/person?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}&page=1`;
...
next.addEventListener("click", () => {
    ...
    peopleContainer.innerHTML = "";
    if(peopleContainer.getAttribute("search-text") === null){
        const url = `https://api.themoviedb.org/3/person/popular?api_key=845e81600d9d504ee9e6e2f1b08dee9f&page=${index}`;
        showPeople(url);
    }else{
        const searchText = peopleContainer.getAttribute("search-text");
        const url = `https://api.themoviedb.org/3/search/person?api_key=845e81600d9d504ee9e6e2f1b08dee9f&query=${searchText}&page=${index}`;
        showPeople(url);
    }
});
